...
route.post("/", bodyParser.json({ limit: "5mb" }), async (req, res) => {
    ...
    if (screenshot) {
        fs.writeFileSync(__dirname + `/../../../CDN_Files/img/screenshots/${post_id}.${(screenshot_MIME.split("/"))[1]}`, screenshot, 'base64');
        const screenshot_result = await cdn_upload.uploadImage(__dirname + `/../../../CDN_Files/img/screenshots/${post_id}.${(screenshot_MIME.split("/"))[1]}`, "screenshots");
